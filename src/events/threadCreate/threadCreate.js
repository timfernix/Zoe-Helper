...
module.exports = async (thread, client, handler) => {
  ...
  if (trigger) {
    ...
    if(thread.parentId === process.env.PARENT_SUPPORT_ID){
      ...
      if(thread.parentId === process.env.PARENT_FEEDBACK_ID){
        ...
        console.log('Parent Channel incorrect - No action taken')
        return
      }
  }
